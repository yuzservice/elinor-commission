...
## نصب و راه‌اندازی آسان با یک تک‌دستور (One-Liner Install with Docker & SSL)

برای کلون، ساخت خودکار داکر، دریافت خودکار SSL و راه‌اندازی کامل سامانه روی سرور Ubuntu، کافی است این تک‌دستور را در ترمینال سرور اجرا کنید:

```bash
bash -c "$(curl -fsSL https://raw.githubusercontent.com/yuzservice/elinor-commission/main/install.sh)"
```

این اسکریپت به صورت خودکار:
۱. پروژه را مستقیماً از گیت‌هاب کلون می‌کند.
۲. نصب بودن Docker و Compose را بررسی و در صورت نیاز روی اوبونتو نصب می‌کند.
۳. در صورت داشتن دامنه، گواهی امنیتی **SSL / HTTPS** را خودکار فعال و دائمی می‌سازد.
۴. فایل محیطی `.env` را با پسوردهای امنیتی قوی ایجاد می‌کند.
۵. کانتینرها را اجرا کرده، مهاجرت‌های دیتابیس را اعمال و کاربر ادمین را می‌سازد.

## بروزرسانی آسان و امن سامانه روی سرور (Safe Zero-Downtime Update)

برای بروزرسانی پنل به آخرین نسخه گیت‌هاب بدون از دست رفتن دیتای واقعی، دستور زیر را در سرور اجرا کنید:

```bash
./update.sh
```

یا با تک‌دستور از هر مسیری روی سرور:
```bash
bash -c "$(curl -fsSL https://raw.githubusercontent.com/yuzservice/elinor-commission/main/update.sh)"
```

این اسکریپت به صورت خودکار:
۱. پیش از هر کاری یک بکاپ اضطراری از دیتابیس در `backups/` ذخیره می‌کند.
۲. کدهای جدید را از گیت‌هاب دریافت و کانتینر را بازسازی می‌کند.
۳. مهاجرت‌های دیتابیس (`migrate`) را روی اطلاعات فعلی بدون پاک شدن داده‌ها اعمال می‌کند.

## پشتیبان‌گیری و بازیابی اطلاعات از طریق پنل (In-UI Backup & Restore)

مدیران می‌توانند از مسیر **«تعاریف و تنظیمات > 💾 پشتیبان‌گیری و بازیابی»** در پنل:
- فایل فشرده ZIP شامل دیتابیس کامل و فایل‌های آپلودشده را ایجاد و به طور آنی دانلود کنند.
- هر زمان لازم بود، فایل ZIP را آپلود و بازیابی (Restore) کنند. سیستم به صورت خودکار مایگریشن‌ها را اجرا کرده و داده‌ها را با نسخه جدید هماهنگ می‌کند.
