...
MVP واقعی و فارسی برای ثبت فعالیت روزانه، تأیید سرپرست، تخلفات و محاسبه شفاف پورسانت. محیط توسعه Mac و استقرار Ubuntu از یک Docker Compose و یک PostgreSQL استفاده می‌کنند.

## امکانات MVP

- ورود امن و سه نقش مدیر، سرپرست و کارمند
- کارکنان چندبخشی و سطح‌های A تا D
- تعریف فعالیت چندبخشی با سه روش امتیازدهی و قابلیت فعال/غیرفعال‌سازی
- ثبت فعالیت، پیش‌نویس، مدرک امن تصویر/PDF و وضعیت‌های تأیید/رد/اصلاح
- صف بررسی مدیر و الزام دلیل برای رد یا اصلاح
- ثبت تخلف با امتیاز پلکانی سه مرتبه
- قوانین قابل تنظیم فعالیت، سطح، ضریب، تخلف و تارگت
...
مدیر از `/management/activity-types/` تعریف فعالیت را با دسته، واحد، روش امتیازدهی، محدودیت مقدار/ثبت روزانه، بخش‌های مجاز و الزام مدرک مدیریت می‌کند. صف بررسی در `/management/activity-reviews/` قرار دارد. کارمند از `/activities/new/` فقط برای خودش ثبت می‌کند و تاریخچه هر رکورد در `/activities/<id>/` قابل مشاهده است. فایل مدرک حداکثر ۵ مگابایت و فقط JPEG، PNG، WebP یا PDF معتبر است.

منطق فعلی: امتیاز هر رکورد مطابق روش تعریف آن و فقط در سرور محاسبه می‌شود. فقط فعالیت تأییدشده و مشمول پورسانت محاسبه می‌شود. `پورسانت = امتیاز × ضریب سطح − امتیاز تخلف × ضریب تخلف + آخرین پاداش تارگت کسب‌شده` و نتیجه منفی صفر نمایش داده می‌شود. ضریب صبح ذخیره شده اما تا روشن‌شدن قاعده دقیق شیت هنوز در محاسبه دخالت ندارد.

## اجرای اولین‌بار روی Mac
