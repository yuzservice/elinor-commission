...
class ActivityForm(forms.ModelForm):
    ...
    evidence = forms.FileField(label="مدرک", required=False, validators=[FileExtensionValidator(["jpg","jpeg","png","webp","pdf"], "فقط تصویر یا PDF مجاز است.")])
    class Meta:
        model = Activity
        fields = ["activity_type", "activity_date", "value", "employee_note", "evidence"]
        widgets = {"employee_note": forms.Textarea(attrs={"rows":3})}
    def __init__(self, *args, employee=None, **kwargs):
        super().__init__(*args, **kwargs)
        self.employee = employee
        self.fields["activity_type"].label = "نوع فعالیت"
        self.fields["activity_date"].initial = jdatetime.date.fromgregorian(date=timezone.localdate()).strftime("%Y/%m/%d")
        qs=ActivityType.objects.filter(active=True).select_related("category")
        if employee and employee.role == Employee.Role.EMPLOYEE:
            qs=qs.filter(Q(all_departments=True)|Q(departments__in=employee.departments.all())|Q(departments=employee.primary_department)).distinct()
        self.fields["activity_type"].queryset=qs
    def clean_evidence(self):
        file=self.cleaned_data.get("evidence")
        if file and file.size > 5*1024*1024: raise forms.ValidationError("حداکثر حجم فایل ۵ مگابایت است.")
        if file:
            extension=file.name.rsplit(".",1)[-1].lower()
            try:
                if extension == "pdf":
                    if file.read(5) != b"%PDF-": raise forms.ValidationError("فایل PDF معتبر نیست.")
                else:
                    image=Image.open(file); image.verify()
                    allowed={"jpg":"JPEG","jpeg":"JPEG","png":"PNG","webp":"WEBP"}
                    if image.format != allowed.get(extension): raise forms.ValidationError("نوع واقعی تصویر با پسوند فایل سازگار نیست.")
        ...
        return file
    def clean(self):
        data=super().clean(); kind=data.get("activity_type"); value=data.get("value"); evidence=data.get("evidence") or getattr(self.instance,"evidence",None)
        if not kind: return data
        if value is None: data["value"]=Decimal("1")
        value=data.get("value")
        if kind.minimum_value is not None and value < kind.minimum_value: self.add_error("value",f"حداقل مقدار {kind.minimum_value} است.")
        if kind.maximum_value is not None and value > kind.maximum_value: self.add_error("value",f"حداکثر مقدار {kind.maximum_value} است.")
        if kind.requires_evidence and not evidence: self.add_error("evidence","برای این فعالیت بارگذاری مدرک الزامی است.")
        if not kind.allow_employee_note: data["employee_note"]=""
        return data
...
class ActivityTypeForm(forms.ModelForm):
    class Meta:
        model=ActivityType
        fields=["title","code","category","description","unit","scoring_method","score_value","multiplier","is_commission_eligible","requires_manager_approval","requires_evidence","allow_employee_note","recurrence_type","max_daily_submissions","minimum_value","maximum_value","all_departments","departments","active","sort_order"]
